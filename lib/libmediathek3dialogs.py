...
def dialogDate(format='%d%m%Y'):
    ...
    result = dialog.numeric(1, libmediathek3utils.getTranslation(
        31030)).replace("/", "").replace(" ", "0")
    try:
...
def getSearchString():
    dialog = xbmcgui.Dialog()
    d = dialog.input(libmediathek3utils.getTranslation(
        31039), type=xbmcgui.INPUT_ALPHANUM)
    return quote_plus(d)
